package com.yit.deploy.plugin.util;

import com.google.gson.Gson;
import com.yit.deploy.core.model.JsonSupport;
import org.kohsuke.stapler.HttpResponse;
import org.kohsuke.stapler.StaplerRequest;
import org.kohsuke.stapler.StaplerResponse;

import javax.servlet.ServletException;
import java.io.IOException;

/**
 * POJO converted to JSON via Jackson as HTTP response.
 *
 * @author Kohsuke Kawaguchi
 */
public class JsonResponse implements HttpResponse {
    public final Object object;
    private static final Gson DEFAULT_GSON = new Gson();


    public JsonResponse(Object object) {
        this.object = object;
    }

    @Override
    public void generateResponse(StaplerRequest req, StaplerResponse rsp, Object node) throws IOException {
        rsp.setContentType("application/json; charset=UTF-8");
        if (object instanceof JsonSupport) {
            ((JsonSupport) object).toJson(rsp.getWriter());
        } else if (object != null) {
            DEFAULT_GSON.toJson(object, object.getClass(), DEFAULT_GSON.newJsonWriter(rsp.getWriter()));
        }
    }
}
